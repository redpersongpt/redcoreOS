  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1440px] px-6 sm:px-8 lg:px-16 2xl:px-24">
        <motion.div className="pt-24 lg:pt-32 max-w-[680px]" style={{ opacity: contentOpacity }}>
          {/* Animated logo mark */}
          <motion.div
          ...
          >
            <svg width={64} height={64} viewBox="0 0 100 100" fill="none">
              ...
              <circle cx="77.1" cy="39.9" r={4} fill="#D42A45" />
            </svg>
          </motion.div>
          ...
          <h1 className="font-bold leading-[0.96]" style={{ fontSize: "clamp(2.8rem, 5.5vw, 5.2rem)" }}>
            ...
            <motion.span
              ...
              style={{ color: "#D42A45", fontFamily: "var(--font-doto), monospace" }}
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          >
            <button
              ...
              className="inline-flex items-center gap-2.5 rounded-full bg-[#D42A45] px-8 py-4 text-[0.92rem] font-semibold text-white cursor-pointer transition-colors duration-200 hover:bg-[#B52238]"
            >
          ...
          >
            {["OPEN SOURCE", "WIN 10 & 11", "SNAPSHOTS", "NO SUBSCRIPTION"].map((t, i) => (
              <span key={t} className="flex items-center gap-3 text-[0.65rem] font-mono font-medium tracking-[0.12em] text-[var(--color-ink-tertiary)]">
                {i > 0 && <span className="h-3 w-px bg-[#D42A45]" />}
                {t}
              </span>
            ))}
          </motion.div>
