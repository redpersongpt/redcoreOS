  ...
  return (
    <div className="rounded-lg border border-white/[0.06] overflow-hidden">
      ...
      >
        ...
        <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-ink-muted transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
            ...
            {phase.actions.map((action) => {
              ...
              return (
                <div key={action.id} className="px-4 py-2">
                  ...
                  {/* Expert rationale — "why this matters" */}
                  {rationale.why ? (
...
export function PlaybookReviewStep() {
  ...
  if (loading) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        ...
        <p className="text-[12px] text-ink-secondary">Resolving playbook...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Context bar: profile + preset + playbook version */}
      ...
      <div className="mb-4 grid grid-cols-4 gap-2">
        {[
          ...
          <div key={label} className={`rounded-lg border border-white/[0.06] ${bg} px-2.5 py-2 text-center`}>
      ...
      {/* Reboot notice */}
      {rebootCount > 0 && (
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-amber-500/20 bg-amber-500/[0.04] px-3 py-2">
      ...
      {/* Blocked reasons summary (Work PC) */}
      {resolvedPlaybook.blockedReasons.length > 0 && (
        <div className="mb-3 rounded-lg border border-red-500/15 bg-red-500/[0.03] px-3 py-2">
      ...
      <div className="space-y-1.5">
        {resolvedPlaybook.phases.map((phase, i) => (
          <PhaseSection key={phase.id} phase={phase} defaultOpen={i === 0} profile={resolvedPlaybook.profile} />
        ))}
      </div>
    </motion.div>
  );
