...
export function PlaybookStrategyStep() {
  ...
  const questions = useMemo<QuestionDefinition[]>(() => [
    ...
    makeBooleanQuestion(
      ...
      "redcore OS only asks this question at the level it can actually apply today.",
    ...
    makeBooleanQuestion(
      ...
      "This is not a fake fixed 0.5 ms timer lock. It applies the real GlobalTimerResolutionRequests path in the playbook.",
      (state) => state.aggressionPreset !== "conservative",
      "Latency",
      "bg-brand-500/15 text-brand-400",
    ),
    ...
    makeBooleanQuestion(
      ...
      "This is the kind of preservation decision that should be explicit before apply, not discovered after something breaks.",
    ),
    {
      key: "edgeBehavior",
      icon: Globe,
    ...
    makeBooleanQuestion(
      ...
      "Only shown on supported Windows builds where Recall actually exists.",
      (_state, ctx) => ctx.windowsBuild >= 26100,
    ),
    makeBooleanQuestion(
      "disableClickToDo",
      Eye,
      ...
      "Only shown on supported Windows builds where Click to Do is present.",
      (_state, ctx) => ctx.windowsBuild >= 26100,
    ),
    makeBooleanQuestion(
      "disableAiApps",
    ...
    makeBooleanQuestion(
      ...
      "Only shown on Windows builds that expose the taskbar End Task toggle.",
      (_state, ctx) => ctx.windowsBuild >= 22631,
    ),
    makeBooleanQuestion(
    ...
    makeBooleanQuestion(
      ...
      "This should be benchmark-first, not dogma-first. redcore OS asks it explicitly and only applies the real mitigation action it actually has.",
