...
async fn export_package(
    ...
    let playbook_root = match playbook_root {
        Some(p) => p,
        None => return Ok(serde_json::to_value(apbx::ExportResult::err("Playbook directory not found"))
    ...
    let output_path = match file_path {
        ...
        None => return Ok(serde_json::to_value(apbx::ExportResult::err("Export cancelled"))
            .map_err(|e| e.to_string())?),
    };
    ...
    match apbx::create_bundle(&output_path, &playbook_root, &wizard_metadata, &state, version, commit) {
        ...
        Err(e) => serde_json::to_value(apbx::ExportResult::err(e)).map_err(|e| e.to_string()),
    }
}
